...
<html lang="zh-cmn-Hans">
    ...
    <body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white">
        ...
        <div class="page-container">
            <!-- BEGIN SIDEBAR -->
            <% request.setAttribute("servlet.menu.sidebar.uri", "/goods/sort/sales/list") ;%>
            ...
            <div class="page-content-wrapper">
                ...
                <div class="page-content">
                    ...
                    <div class="note note-info">
                        <p> 商品分类列表，商品分类的添加编辑等操作 </p>
                    </div>
                    
                   <%-- 
                    <div class="row">
                       <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                         	<button id="modal_ajax_demo_btn" class="btn green" onclick="javascript:BeeModel.openModel('/system/sort/to');">添加主菜单 <i class="icon-plus"></i></button>
                       </div>
                    </div>
                    <div class="clearfix"> </div>
                   --%>
                   ...
                   <div class="row">
		                   <c:forEach items="${clist}" var="c">
					        ...
					        <h3 style="margin:10px 0;clear:both;"><span class="shi" onclick="cg('${c.id}_div',this)">&nbsp;&nbsp;</span>${c.sortName} <a href="javascript:void(0);" onclick="javascript:BeeModel.openModel('/goods/sort/sales/to?sortId=${c.id}');"   style="width:80px;font-weight:normal;font-size:12px;">编辑</a> <a href="javascript:void(0);" onclick="javascript:BeeModel.openModel('/goods/sort/to?pid=${c.id}');"   style="width:80px;font-weight:normal;font-size:12px;">添加子菜单</a> <span style="color:#ff6666;font-weight:normal;font-size:12px;">${c.sortSeq}</span></h3>
					        <dl style="padding-left:0px;display:none;" id="${c.id}_div">
						        <c:forEach items="${c.list}" var="m">
						                <dt style="margin:0;clear:both;padding:0;"><span class="yi">&nbsp;&nbsp;</span><span class="shi" onclick="bcg('dd',${m.id},this)">&nbsp;&nbsp;</span>${m.sortName} <span style="color:#ff6666;">${son.sortSeq}</span>
						                &nbsp;<a href="javascript:void(0);" onclick="javascript:BeeModel.openModel('/goods/sort/sales/to?sortId=${m.id}');"   style="width:80px;">编辑</a>
						                &nbsp;<a href="javascript:void(0);" onclick="javascript:BeeModel.openModel('/goods/sort/sales/to?pid=${m.id}');"   style="width:80px;">添加子菜单</a></dt>
						                
						                <c:forEach items="${m.list}" var="son">
						                       <dd parent="${m.id}" style="margin:0;clear:both;padding:0; display:none; text-indent:0px;"><span class="yi"></span><span class="yi"></span> <c:choose><c:when test="${fn:length(son.list)>0}"><span class="shi" onclick="bcg('dd',${son.id},this);"></span></c:when><c:otherwise><span class="yi">&nbsp;&nbsp;</span></c:otherwise></c:choose>${son.sortName} <span style="color:#ff6666;">${son.sortSeq}</span>
						                       &nbsp;<a href="javascript:void(0);" onclick="javascript:BeeModel.openModel('/goods/sort/sales/to?sortId=${son.id}');"   style="width:80px;">编辑</a>
						                       </dd> 
						                </c:forEach>
						        </c:forEach>
					        </dl>
		                ...
		                form1.validate({
		    	             ...
		    	             ignore: "",
		    	                ...
		    	                submitHandler: function (form) {
		    	                    ...
		    	                    Xhr.cfg("/goods/sort/sales/su",{},function(response){
