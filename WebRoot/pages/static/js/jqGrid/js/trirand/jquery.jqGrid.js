	...
	return this.each( function() {
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if (ts.p.viewrecords){
					if(ts.p.reccount === 0) {
						$(ts).prev("div").html($.jgrid.getRegional(ts, "defaults.emptyrecords", ts.p.emptyrecords )).addClass("no-data-box");
						$(".ui-paging-info",pgboxes).html($.jgrid.getRegional(ts, "defaults.emptyrecords", ts.p.emptyrecords ));
					} else {
                        $(ts).prev("div").removeClass("no-data-box").html("");
						from = base+1;
						tot=ts.p.records;
						if($.fmatter) {
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0){
				str = "<td dir=\""+dir+"\">";
				str +="<select "+getstyle(stylemodule, 'pgSelectBox', false, 'ui-pg-selbox')+" role=\"listbox\" style=\"border:.1rem solid #ddd;height:16px;width: auto;font-size:.9em; margin:0;line-height: inherit; padding:0 1px;\" title=\""+($.jgrid.getRegional(ts,"defaults.pgrecs",ts.p.pgrecs) || "")+ "\">";
				var strnm;
			...
			if(ts.p.pginput===true) {
				$("#"+pgcnt).on('keypress','input.ui-pg-input', function(e) {
					...
					if(key === 13) {
						...
						var num = intNum( $(this).val(), 1);
						num = num > 0 ? num : 1;
						$(this).val( num > ts.p.lastpage ? ts.p.lastpage : num);
