 ...
 */
 ...
 function Page(action,callback,fdiv){
     var T = this;
     T.totalRows = 0;//总行数
     T.pageSize = 5;//每页显示行数
     T.totalPage = 0;
   ...
   //设置页脚 01
     T.setFoot_01 = function(footDiv){ 
         ...
         f.appendChild(tol);
         //上一页
         var prev  = util.cElem("a",{href:"javascript:void(0);",className:(foot.currentPage==1)?"disabled":""},'上一页');
         prev.onclick = util.call(T.prePage,T);
         f.appendChild(prev);
         //页码
         for(;b<=e;b++){
         ...
         //下一页
         var next  = util.cElem("a",{href:"javascript:void(0);"},'下一页');
         (T.current!=foot.totalpage)&&(next.onclick = util.call(T.nextPage,T));
         (T.current==foot.totalpage)&&(next.disabled='disabled');
         f.appendChild(next);
